...
class LinkedList
{
    ...
    // this function removes Nth node from beginning of LL
    public void removeNthNodefromBeginning(int n)
    {
        // this is case when we are to delete first node
        if (n == 0)
        {
            head = head.next;
            size--;
        ...
        // let prev point to head
        Node prev = head;
        // this iterator start with 0
        int i = 0;
        // we iterate till we prev node point to Nth node
        while (i < n - 1)
        {
            prev = prev.next;
            i++;
    ...
    public static void main(String[] args)
    {
        ...
        // inputing Nth node from last
        int LN = sc.nextInt();
        // printing LL initially
        ll.printLL();
        // removinf Nth node from last is equivalent to remove (size - N) from beginning
        ll.removeNthNodefromBeginning(size - LN);
        // final print after removal
        ll.printLL();
    }
}
